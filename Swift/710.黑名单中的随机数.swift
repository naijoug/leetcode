...
class Solution {
    ...
    init(_ n: Int, _ blacklist: [Int]) {
        var blackDict = [Int: Bool]()
        for num in blacklist {
            blackDict[num] = true
        }
        var nums = [Int]()
        for num in 0..<n {
            guard blackDict[num] == nil else { continue }
            nums.append(num)
        }
        self.nums = nums
    }
    ...
    func pick() -> Int {
        nums[Int.random(in: 0..<nums.count)]
    }
}
...
// let solution = Solution(7, [2, 3, 5]) 
let solution = Solution(1000000000, [640145908])
// [0, 1, 4, 6]
print("random1:")
print(solution.pick())
print(solution.pick())
